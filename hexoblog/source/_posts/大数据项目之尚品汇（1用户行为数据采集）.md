---
title: 大数据项目之尚品汇数仓4.0
date: 2022-02-01 15:03:13
...
{% asset_img 业务数据.png This is an example image %}

用户行为数据：用户在使用产品过程中，通过埋点收集与客户端产品交互过程中产生的数据，并发往日志服务器进行保存。比如页面浏览、点击、停留、评论、点赞、收藏等。用户行为数据通常存储在日志文件中。

{% asset_img 用户行为数据.png This is an example image %}

爬虫数据：通常事通过技术手段获取其他公司网站的数据。不建议同学们这样去做。

{% asset_img 爬虫数据.png This is an example image %}

## 第2章 项目需求及架构设计
### 2.1 项目需求分析

{% asset_img 项目需求分析.png This is an example image %}

### 2.2 项目框架
#### 2.2.1 技术选型

{% asset_img 技术选型.png This is an example image %}

#### 2.2.2 系统数据流程设计

{% asset_img 系统数据流程设计.png This is an example image %}

#### 2.2.3 框架版本选型

{% asset_img 框架版本选型.png This is an example image %}

{% asset_img 具体版本号.png This is an example image %}
...
{% asset_img 集群规模.png This is an example image %}
...
{% asset_img 页面.png This is an example image %}
...
{% asset_img 事件.png This is an example image %}
...
{% asset_img 曝光.png This is an example image %}
...
{% asset_img 启动.png This is an example image %}
...
{% asset_img XShell工具.png This is an example image %}
...
{% asset_img login shell和non-login shell.png This is an example image %}

这两种shell的主要区别在于，它们启动时会加载不同的配置文件，login shell启动时会加载/etc/profile，~/.bash_profile，~/.bashrc。non-login shell启动时会加载~/.bashrc。

而在加载~/.bashrc（实际是~/.bashrc中加载的/etc/bashrc）或/etc/profile时，都会执行如下代码片段，

{% asset_img 加载bashrc.png This is an example image %}
...
[大数据技术之Hadoop（入门）](https://bigworldxld.github.io/2022/02/02/undefined.html)
...
2. 生产环境服务器磁盘情况
   ...
   {% asset_img 服务器磁盘情况.jpg This is an example image %}
...
{% asset_img HDFS的读写性能.png This is an example image %}

HDFS的读写性能主要受***\*网络和磁盘\****影响比较大。为了方便测试，将hadoop102、hadoop103、hadoop104虚拟机网络都设置为100mbps。

100Mbps单位是bit；10M/s单位是byte ; 1byte=8bit，100Mbps/8=12.5M/s。

{% asset_img 网络和磁盘.png This is an example image %}
...
1. 测试HDFS写性能
   ...
   + 写测试底层原理
     ...
     {% asset_img HDFS写性能.png This is an example image %}
...
+ 测试结果分析
  ...
  {% asset_img 副本1就在本地.png This is an example image %}
...
{% asset_img 三个副本都参与.png This is an example image %}
...
2. 测试HDFS读性能
   ...
   ```shell
   [atguigu@hadoop102 mapreduce]$ hadoop jar /opt/module/hadoop-3.1.3/share/hadoop/mapreduce/hadoop-mapreduce-client-jobclient-3.1.3-tests.jar 
   ```
...
{% asset_img 三台服务器，且有三个副本.png This is an example image %}
...
[大数据技术之Zookeeper](https://bigworldxld.github.io/2022/02/03/undefined.html)
...
{% asset_img Kafka安装.png This is an example image %}

#### 4.4.1 Kafka集群安装

详见：大数据技术之Kafka

[大数据技术之Kafka](https://bigworldxld.github.io/2022/02/04/undefined.html)
...
{% asset_img Kafka Producer压力测试.png This is an example image %}
...
{% asset_img Kafka Consumer压力测试.png This is an example image %}
...
{% asset_img Flume.png This is an example image %}

#### 日志采集Flume安装

详见：大数据技术之Flume

[大数据技术之Flume](https://bigworldxld.github.io/2022/02/05/undefined.html)
...
{% asset_img Flume配置分析.png This is an example image %}
...
{% asset_img Flume-Kafka通道.png This is an example image %}
...
{% asset_img FileChannel优化.png This is an example image %}
